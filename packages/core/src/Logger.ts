...
export class VoerkaLogger {
    ...
    #cache: [any, any][] = [];
    attached: boolean = false; // 当libraay=true时，是否已附加到全局
    constructor(options?: VoerkaLoggerConstructorOptions) {
        const isLibrary = options?.library === true;
        if ((globalThis as any).logger) {
            return (globalThis as any).logger;
        }
        // 通过异步信号来处理库与应用的初始化顺序问题
        //@ts-expect-error
        const signals = (globalThis.__VOERKA_LOGGER_SIGNALS___ || []) as IAsyncSignal[];
        if (isLibrary === true) {
            ...
            signals.push(signal);
            //@ts-expect-error
            globalThis.__VOERKA_LOGGER_SIGNALS___ = signals;
            // 禁用：禁用时写入的数据会先写入cache
            this.enable = false;
        } else {
            signals.forEach((signal) => signal());
            signals.splice(0, signals.length);
            //@ts-expect-error
            delete globalThis.__VOERKA_LOGGER_SIGNALS___;
        }
    ...
    get levelName() {
        return VoerkaLoggerLevelNames[this.options.level];
    }
    get level() {
        return this.options.level as VoerkaLoggerLevel;
    }
    ...
    get transports() {
        return this._transportInstances;
    }
