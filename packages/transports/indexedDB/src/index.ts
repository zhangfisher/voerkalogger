import { TransportBaseOptions, TransportBase, TransportOptions, LogMethodVars, VoerkaLoggerRecord } from '@voerkalogger/core';
...
export default class IndexedDbTransport<Output = string> extends TransportBase<IndexedDbOptions<Output>> {
    #storage: any;
    ...
    // 添加日志
    private async _addLog(log: { level: string; message: string; created: string; module: string }) {
        // 生成唯一 ID
    ...
    getLogs(query: any) {
        return this.getLogsPaginated(query) as any;
    }
    // 分页查询日志
    private async getLogsPaginated({ page = 1, pageSize = 10, level = '' }: { page: number; pageSize: number; level: string }) {
        // 获取所有日志的键名
        const keys = await this.#storage.getKeys(`${this.options.dbName}:::logs:data::`);
        // 修复键名（如果冒号被合并）
        const fixedKeys = keys.map((key: any) => key.replace(`${this.options.dbName}:`, ''));
        // 获取所有日志数据
        ...
        // 按创建时间降序排序
        const sortedLogs = filteredLogs.sort((a, b) => b.created.localeCompare(a.created));
        // 计算分页范围
    ...
    private async cleanupOldLogs() {
        ...
        const fixedKeys = keys.map((key: any) => key.replace(`${this.options.dbName}:`, ''));
        // 获取所有日志数据
        const logs = await Promise.all(fixedKeys.map(async (key: any) => await this.#storage.getItem(key)));
        // 过滤无效数据并按时间升序排序（旧日志在前）
        const validLogs = logs.filter(Boolean);
        const sortedLogs = validLogs.sort((a, b) => a.created.localeCompare(b.created));
        // 计算需要删除的数量
     ...
     * @param pageSize 每页大小
     */
    async exportLogsPaginated({ page = 1, pageSize = 10, level = '' }: { page: number; pageSize: number; level: string }): Promise<string> {
        // 获取分页日志数据
     ...
     * @param pageSize 每页大小
     */
    async exportLogsToFile({ page = 1, pageSize = this.options.maxFileCount, level = '' }: { page: number; pageSize: number; level: string }): Promise<void> {
    ...
    async clear() {
        // 获取所有日志数据的键名
        const logKeys = await this.#storage.getKeys(`${this.options.dbName}:::logs:data::`);
        const fixedLogKeys = logKeys.map((key: any) => key.replace(`${this.options.dbName}:`, ''));
        // 获取所有索引的键名
        const indexKeys = await this.#storage.getKeys(`${this.options.dbName}:::logs::index::`);
        const fixedIndexKeys = indexKeys.map((key: any) => key.replace(`${this.options.dbName}:`, ''));
        // 删除所有日志数据
        await Promise.all(fixedLogKeys.map((key: any) => this.#storage.removeItem(key)));
        // 删除所有索引
        await Promise.all(fixedIndexKeys.map((key: any) => this.#storage.removeItem(key)));
