...
import type { VoerkaLoggerRootScopeOptions } from './scope';

// 日志配置，用于构建VoerkaLogger实例时使用，全部使用可选参数
export interface VoerkaLoggerConstructorOptions {
    id?: string; // 当前应用ID
    enable?: boolean; // 全局开关
    scope?: VoerkaLoggerRootScopeOptions; // 用来标识当前应用，如设备所在的IP地址，或者用户名称等
...
// 对日志记录进行格式化以便输出到后端
export type VoerkaLoggerFormatter<Output = VoerkaLoggerRecord> = (record: VoerkaLoggerRecord, vars: LogMethodVars) => Output;
